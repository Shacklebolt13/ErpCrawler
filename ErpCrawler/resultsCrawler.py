...
def resultsCrawler(username, password, semester, cookies):
    ...
    def semesterResultDetails(response):
        ...
        table = pandas.read_html(
            etree.tostring(
                tree.xpath(r'//*[@id="ctl00_cpStud_grdSemwise"]')[0], pretty_print=True
            )
        )[0]
        ...
        try:
            sgpa = etree.tostring(
                tree.xpath(r'//*[@id="ctl00_cpStud_lblSemSGPA"]')[0], pretty_print=True
            )
        ...
        try:
            cgpa = etree.tostring(
                tree.xpath(r'//*[@id="ctl00_cpStud_lblSemCGPA"]')[0], pretty_print=True
            )
    ...
    for k, v in cookies.items():
        session.cookies.set_cookie(
            requests.cookies.create_cookie(
                name=k, value=v, domain="www.gandhionline.in"
            )
        )
