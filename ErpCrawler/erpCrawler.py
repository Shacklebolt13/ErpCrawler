...
class ErpCrawler(scrapy.Spider):
    ...
    def parse(self, response: HtmlResponse):
        ...
        yield scrapy.FormRequest(
            url=self.attendanceSite, formdata=self.data, callback=self.loginWithPassword
        )
    ...
    def loginWithPassword(self, response: HtmlResponse):
        ...
        yield scrapy.FormRequest(
            url=self.attendanceSite, formdata=self.data, callback=self.gotoSemMarksPage
        )
    ...
    def hasThisSemester(self, response: HtmlResponse):
        if f"{self.roman(self.semester,increment=False)} SEMESTER" in str(
            response.body
        ):
            return True
        else:
    ...
    def getBasicData(self, response, reset=False):
        ...
        self.__VIEWSTATEGENERATOR = response.css(
            'input[name="__VIEWSTATEGENERATOR"]::attr(value)'
        ).extract_first()
        self.__EVENTVALIDATION = response.css(
            'input[name="__EVENTVALIDATION"]::attr(value)'
        ).extract_first()
        self.__VIEWSTATE = response.css(
    ...
    def getThisSemMarks(self, response: HtmlResponse, semester):
        ...
        crystalState = response.css(
            'input[name="__CRYSTALSTATEctl00$cpStud$CrystalReportViewer1"]::attr(value)'
        ).extract_first()
        self.data.update(
            {
                f"ctl00$cpStud$btn{semester}": f"{self.roman(semester)} SEMESTER",
                "__CRYSTALSTATEctl00$cpStud$CrystalReportViewer1": crystalState,
            }
        )
        yield scrapy.FormRequest(
            url=self.marksSite, formdata=self.data, callback=self.getMarks
        )

    def getMarks(self, response: HtmlResponse):
        if self.semester > 1:
            self.returnJson.update(
                {f"{self.semester-1} semester": self.percentageDetails(response)}
            )
    ...
    def attendanceDetails(self, response: HtmlResponse):
        table = pandas.read_html(
            "".join(
                response.xpath(r'//*[@id="ctl00_cpStud_grdSubject"]').extract_first()
            )
        )[0]
        att = table.iloc[-1, -1]
        ...
        table2 = pandas.read_html(
            "".join(
                response.xpath(r'//*[@id="ctl00_cpStud_grdDaywise"]').extract_first()
            )
        )[0]
    ...
    def percentageDetails(self, response: HtmlResponse):
        table = pandas.read_html(
            "".join(response.xpath(r'//*[@id="ctl00_cpStud_grdSemwise"]').extract())
        )[0]
